...
export function resolveSeriesDetailsBgTheme(seriesId?: string): SeriesDetailsBgTheme {
  const global = getGlobalSeriesDetailsBgTheme();
  if (!seriesId) return global;
  const seriesOverride = getSeriesDetailsBgThemeOverride(seriesId);
  return mergeTheme(DEFAULT_SERIES_DETAILS_BG_THEME, global, seriesOverride);
}
...
export function resolveSeriesDetailsBackground(options: {
  ...
  const theme = resolveSeriesDetailsBgTheme(options.seriesId);
