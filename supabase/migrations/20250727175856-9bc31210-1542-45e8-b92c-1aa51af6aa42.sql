...
-- Add RLS policies to prevent placeholder content on existing tables
DROP POLICY IF EXISTS "prevent_placeholder_manga_meta_title" ON manga_meta;
CREATE POLICY "prevent_placeholder_manga_meta_title" ON manga_meta
FOR INSERT WITH CHECK (NOT public.contains_placeholder_content(title));

DROP POLICY IF EXISTS "prevent_placeholder_manga_meta_description" ON manga_meta;
CREATE POLICY "prevent_placeholder_manga_meta_description" ON manga_meta
FOR INSERT WITH CHECK (description IS NULL OR NOT public.contains_placeholder_content(description));

DROP POLICY IF EXISTS "prevent_placeholder_chapters_title" ON chapters;
CREATE POLICY "prevent_placeholder_chapters_title" ON chapters
FOR INSERT WITH CHECK (title IS NULL OR NOT public.contains_placeholder_content(title));

DROP POLICY IF EXISTS "prevent_placeholder_comments_content" ON comments;
CREATE POLICY "prevent_placeholder_comments_content" ON comments
FOR INSERT WITH CHECK (NOT public.contains_placeholder_content(content));

-- Create a missing system_logs table for logging
