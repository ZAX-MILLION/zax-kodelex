-- Add missing columns to existing user_activity_logs table if they don't exist
DO $$ 
BEGIN
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'user_activity_logs' AND column_name = 'content_id') THEN
    ALTER TABLE public.user_activity_logs ADD COLUMN content_id UUID;
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'user_activity_logs' AND column_name = 'content_type') THEN
    ALTER TABLE public.user_activity_logs ADD COLUMN content_type TEXT;
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'user_activity_logs' AND column_name = 'duration_seconds') THEN
    ALTER TABLE public.user_activity_logs ADD COLUMN duration_seconds INTEGER;
  END IF;
END $$;
